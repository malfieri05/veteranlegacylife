// Test with EXACT same data structure as testNewEntriesAndEmails uses internally
...
async function testExactMatchInternal() {
  console.log('=== TESTING EXACT MATCH WITH INTERNAL testNewEntriesAndEmails DATA ===');
  
  try {
    // Test 1: NEW LEAD Entry - EXACTLY like testNewEntriesAndEmails internal data
    console.log('Test 1: Creating NEW LEAD entry...');
    const leadSessionId = 'TEST_LEAD_INTERNAL_' + Date.now();
    const leadData = {
    ...
    // Test 2: COMPLETE APPLICATION Entry - EXACTLY like testNewEntriesAndEmails internal data
    console.log('Test 2: Creating COMPLETE APPLICATION entry...');
    const appSessionId = 'TEST_APP_INTERNAL_' + Date.now();
    const appData = {
    ...
    console.log('4. Verify quote data columns 36-41 are populated correctly');
